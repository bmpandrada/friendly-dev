        ...
        <nav className="bg-gray-800 border-b border-gray-700 shadow-md sticky top-0 z-50">
            <div className="max-w-6xl mx-auto px-6 py-4 flex justify-between items-center">
                <NavLink to={'/'} className={'flex items-center gap-2 text-lg font-bold text-blue-300'}>
                        <FaLaptopCode className="text-blue-400 text-xl"/><span>BMPA Dev</span>
                </NavLink>
            ...
            { menuOpen && (
                <div className="bg-gray-800 border-t border-gray-700 px-6 py-4 space-x-4 space-y-2 text-center">
                    <NavLink 
                            ...
                            className={({isActive})=> isActive ? active : base } to={'/'}>Home</NavLink>
                    <NavLink 
                            onClick={()=>setMenuOpen(false)}
                            className={({isActive})=> isActive ? active : base } to={'/projects'}>Projects</NavLink>
                    <NavLink 
                            onClick={()=>setMenuOpen(false)}
                            className={({isActive})=> isActive ? active : base } to={'/blog'}>Blog</NavLink>
                    <NavLink 
                            onClick={()=>setMenuOpen(false)}
                            className={({isActive})=> isActive ? active : base } to={'/about'}>About</NavLink>
                    <NavLink 
                            onClick={()=>setMenuOpen(false)}
                            className={({isActive})=> isActive ? active : base } to={'/contact'}>Contact</NavLink>
                </div>
            ) }
        </nav>
     );
